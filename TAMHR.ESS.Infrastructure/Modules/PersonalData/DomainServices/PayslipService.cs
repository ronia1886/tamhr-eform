...
namespace TAMHR.ESS.Infrastructure.DomainServices
{
    ...
    /// </summary>
    public class PayslipService : DomainServiceBase
    {
        ...
        public PayslipService(ConfigService configService, PersonalDataService personalDataService, IUnitOfWork unitOfWork)
            : base(unitOfWork)
        {
        ...
        public async Task<byte[]> Download(string noreg, int month, int period, bool isOffCycle = false, bool viewOnly = true)
        {
            ...
            var parameters = new Dictionary<string, object>
            {
                ...
                ["monthNumber"] = month,
                // Get and set month number with padding zero.
            ...
            // Get and set payslip path (either file path or ftp path) from configuration with given parameters.
            var paySlipPath = StringHelper.Format(_configService.GetConfigValue<string>(Configurations.PayslipPath), parameters);

            // Get and set use ftp indicator from configuration with false as default value.
            var useFtp = _configService.GetConfigValue(Configurations.PayslipUseFtp, defaultValue: false);

            // Get and set default payslip password file from configuration
            // (default password to open the pdf document from file path or ftp path generated by SAP) with given parameters.
            var defaultPayslipPasswordFile = StringHelper.Format(_configService.GetConfigValue(Configurations.PayslipPasswordFile, string.Empty), parameters);

            // Get and set default owner password file from configuration (owner password is used to prevent modify the PDF document).
            var defaultOwnerPassword = _configService.GetConfigValue(Configurations.PayslipOwnerPassword, "Transformer6hap");
            ...
            else
            {
                ...
                password = StringHelper.Format(_configService.GetConfigValue(Configurations.PayslipDefaultPassword, string.Empty), defaultParameters);
            }

            // Get and set filename from configuration with given parameters.
            var fileName = StringHelper.Format(_configService.GetConfigValue<string>(isOffCycle ? Configurations.PayslipOffCycleFilename : Configurations.PayslipFilename), parameters);
            ...
            if (useFtp)
            {
                ...
                var enableSsl = _configService.GetConfigValue(Configurations.PayslipFtpEnableSsl, defaultValue: false);

                // Get and set ftp username from configuration.
                var ftpUsername = _configService.GetConfigValue<string>(Configurations.PayslipFtpUsername);

                // Get and set ftp password from configuration.
                var ftpPassword = _configService.GetConfigValue<string>(Configurations.PayslipFtpPassword);

                // Create URI (Uniform Resource Identifier) from payslip path.
                var uri = new Uri(paySlipPath);
                ...
                var testData = new[] { localPath + "/" + fileName.Trim(new[] { '\\', '/' }) };
                ...
                var testUriSerialize = JsonConvert.SerializeObject(dataObject);
                var testDataSerialize = JsonConvert.SerializeObject(testData);
                new LogService(UnitOfWork).InsertLog("FTP", "Check Files Payslip Name", "Payslip Download", "false", string.Format("test Data :{0}, Uri : {1}", testDataSerialize, testUriSerialize), noreg);
                //client.ConnectTimeout = 120000;
                //client.DataConnectionConnectTimeout = 120000;
                ...
                try
                {
                    ...
                    // If offcycle then filter the files.
                    //if (isOffCycle)
                    //{
                        // Listing all files in remote path.
                        var files = client.GetListing(localPath);

                        // Create new regular expression from file name.
                        ...
                        selectedFiles = files.Where(x => x.Type == FtpFileSystemObjectType.File && regex.IsMatch(x.Name))
                            ...
                            .ToArray();
                    //}
                    var serializeData = JsonConvert.SerializeObject(selectedFiles);

                    if(selectedFiles.Length == 0)
                    {
                        new LogService(UnitOfWork).InsertLog("FTP", "Check Selected Files Payslip", "Payslip Download", "false", string.Format("Serialize Data : {0}",serializeData), noreg);
                    }
                    // Throw an exception if there are no files.
                    Assert.ThrowIf(selectedFiles.Length == 0, "Payslip for this period was not found.");
                    ...
                    using (var masterDocument = new PdfDocument())
                    {
                        ...
                        foreach (var file in selectedFiles)
                        {
                            ...
                            using (var memoryStream = new MemoryStream())
                            {
                                ...
                                using (var doc = PdfReader.Open(memoryStream, defaultPayslipPasswordFile, PdfDocumentOpenMode.Import))
                                {
                ...
                {
                    bool IsError = true;
                    new LogService(UnitOfWork).InsertLog("FTP", "Connect FTP", "Payslip Download", "false", ex.ToString(), noreg);
                    Assert.ThrowIf(IsError,ex.Message);
                }
                
            }
            else
            {
                // Get and set file path from given payslip path and file name.
                var filePath = Path.Combine(paySlipPath, fileName);

                if (!File.Exists(filePath))
                {
                    new LogService(UnitOfWork).InsertLog("FTP", "Check FilePath Payslip", "Payslip Download", "false", filePath, noreg);
                }

                // Throw an exception if PDF file was not found.
                Assert.ThrowIf(!File.Exists(filePath), "Payslip for this period was not found.");

                // Open an existing PDF file for reading.
                using (var file = File.OpenRead(filePath))
                // Create memory stream object.
                using (var ms = new MemoryStream())
                // Open existing PDF document from file stream object with defualt payslip password and mark as modify.
                using (var doc = PdfReader.Open(file, defaultPayslipPasswordFile, PdfDocumentOpenMode.Modify))
                {
        ...
        private void AttachSecuritySetting(PdfDocument pdfDocument, string password, string defaultOwnerPassword)
        {
            ...
            var enablePrinting = _configService.GetConfigValue(Configurations.PayslipEnablePrinting, defaultValue: false);

            // Get and set enable document editing indicator from configuration.
            var enableEditing = _configService.GetConfigValue(Configurations.PayslipEnableEditing, defaultValue: false);
