...
{
    public class APARRefillService : DomainServiceBase
    {
        #region Constructor
        /// <summary>
        /// Public constructor.
        ...
        protected IRepository<APARRefillModel> APARRefillTBRepository => UnitOfWork.GetRepository<APARRefillModel>();
        protected IReadonlyRepository<APARRefillModel> APARRefillReadOnlyTBRepository => UnitOfWork.GetRepository<APARRefillModel>();
        protected IRepository<APARRefillViewModel> APARRefillViewRepository => UnitOfWork.GetRepository<APARRefillViewModel>();
        protected IReadonlyRepository<APARRefillViewModel> APARRefillReadOnlyViewRepository => UnitOfWork.GetRepository<APARRefillViewModel>();
        ...
        #endregion
        public APARRefillService(IUnitOfWork unitOfWork)
            : base(unitOfWork)
        {
        }

        public IQueryable<APARRefillViewModel> GetQuery()
        {
            return APARRefillReadOnlyViewRepository.Fetch()
                .Where(x => x.RowStatus == true)
                .AsNoTracking();
        }

        public IEnumerable<APARRefillViewModel> Gets(string divisionCode = null, string areaId = null, string periode = null, string NoReg = null)
        {
            var query = GetQuery();
        ...
        public IQueryable<APARRefillViewModel> GetAPARRefill(string periode = null, string divisionCode = null, string areaId = null, string NoReg = null)
        {
            if (periode == null)
            {
        ...
        public APARRefillViewModel GetById(Guid id)
        {
            return GetQuery().Where(x => x.Id == id)
                .FirstOrDefaultIfEmpty();
        }
        public void Upsert(APARRefillModel param)
        {
            APARRefillTBRepository.Upsert<Guid>(param, Properties);
            UnitOfWork.SaveChanges();
        }
        public void Delete(Guid id)
        {
            var item = APARRefillTBRepository.Fetch()
                .Where(x => x.Id == id)
                .FirstOrDefault();
            ...
            if (item != null)
            {
                ...
                APARRefillTBRepository.Upsert<Guid>(item, Properties);
                UnitOfWork.SaveChanges();
            }
        }
