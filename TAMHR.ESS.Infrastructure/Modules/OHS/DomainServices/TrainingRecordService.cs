...
{
    public class TrainingRecordService : DomainServiceBase
    {
        #region Constructor
        /// <summary>
        /// Public constructor.
        ...
        protected IRepository<TrainingRecordModel> TrainingRecordTBRepository => UnitOfWork.GetRepository<TrainingRecordModel>();
        protected IReadonlyRepository<TrainingRecordModel> TrainingRecordReadOnlyTBRepository => UnitOfWork.GetRepository<TrainingRecordModel>();
        protected IRepository<TrainingRecordViewModel> TrainingRecordViewRepository => UnitOfWork.GetRepository<TrainingRecordViewModel>();
        protected IReadonlyRepository<TrainingRecordViewModel> TrainingRecordReadOnlyViewRepository => UnitOfWork.GetRepository<TrainingRecordViewModel>();
        ...
        #endregion
        public TrainingRecordService(IUnitOfWork unitOfWork)
            : base(unitOfWork)
        {
        }

        public IQueryable<TrainingRecordViewModel> GetQuery()
        {
            return TrainingRecordReadOnlyViewRepository.Fetch()
                .Where(x => x.RowStatus == true)
                .AsNoTracking();
        }

        public IQueryable<TrainingRecordViewModel> GetTrainingRecord(string periode = null, string divisionCode = null, string areaId = null, string NoReg = null)
        {
            if (periode == null)
            {
        ...
        public IEnumerable<TrainingRecordViewModel> Gets(string divisionCode = null, string areaId = null, string periode = null, string NoReg = null)
        {
            var query = GetQuery();
        ...
        public TrainingRecordViewModel GetById(Guid id)
        {
            return GetQuery().Where(x => x.Id == id)
                .FirstOrDefaultIfEmpty();
        }
        public void Upsert(TrainingRecordModel param)
        {
            TrainingRecordTBRepository.Upsert<Guid>(param, Properties);
            UnitOfWork.SaveChanges();
        }
        public void Delete(Guid id)
        {
            var item = TrainingRecordTBRepository.Fetch()
                .Where(x => x.Id == id)
                .FirstOrDefault();
            ...
            if (item != null)
            {
                ...
                TrainingRecordTBRepository.Upsert<Guid>(item, Properties);
                UnitOfWork.SaveChanges();
            }
        }
