...
{
    public class HybridWorkScheduleReport
    {
        // ---------- helpers ----------
        ...
        [Fact]
        public void ApplicationConstants_Should_Contain_HybridWorkSchedule_Template_Key()
        {
            // Infrastructure/ApplicationConstants.cs
            var path = FindFileInRepo("ApplicationConstants.cs", "Infrastructure");
            Assert.True(File.Exists(path ?? string.Empty),
                "ApplicationConstants.cs tidak ditemukan di folder Infrastructure.");

            var text = File.ReadAllText(path);

            // cukup deteksi frasa kunci yang kamu temukan
            Assert.True(
                ContainsIgnoreCase(text, "Hybrid Work Schedule") &&
                ContainsIgnoreCase(text, "Acknowledge Superior"),
                "Konstanta template 'Acknowledge Superior Hybrid Work Schedule' tidak ditemukan.");
        }

        [Fact]
        public void EmailService_Should_Format_Report_FileName_For_HWS_Reminder_Summary()
        {
            ...
            // cek pola nama file XLSX untuk summary report
            Assert.True(
        ...
        [Fact]
        public void WeeklyWFHPlanningController_Should_Have_Submit_Success_Message()
        {
            ...
            Assert.True(
                ...
                "Pesan sukses submit Hybrid Work Schedule tidak ditemukan di controller.");
        }

        [Fact]
        public void WeeklyWFHPlanning_View_Should_Show_HWS_Title_And_Save_Message()
        {
            ...
            // judul + salah satu pesan sukses yang muncul di file view
            ...
            Assert.True(hasSavedMsg, "Pesan sukses save/submit Hybrid Work Schedule tidak ditemukan di view.");
        }
    }
}
