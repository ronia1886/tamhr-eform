...
namespace TAMHR.ESS.WebUI.Areas.TimeManagement.Controllers
{
    ...
    [Route("api/overtime-log-tracking")]
    public class OvertimeLogTrackingAPIController : FormApiControllerBase<OvertimeLogTrackingStoredEntity>
    {
        ...
        public DataSourceResult GetReport([DataSourceRequest] DataSourceRequest request)
        {
            ...
            var getData = logTrackingService.GetOvertimeLogTracking(startDateDT, endDateDT, NoReg, PostCode, createdDateDT).ToList();
        ...
        public IActionResult DownloadReport(DateTime startDate, DateTime endDate, string employeeName, string listDirectorate, string listDivision, string listDepartment, string listSection, string listLine, string listGroup, string className, string type, string createdName, string noReg, DateTime? createdDate)
        {
            var NoReg = ServiceProxy.UserClaim.NoReg;
            var PostCode = ServiceProxy.UserClaim.PostCode;

            var getData = logTrackingService.GetOvertimeLogTracking(startDate, endDate, NoReg, PostCode, createdDate).ToList();
            ...
            {
                getData = getData.Where(c => c.Type.Contains(type)).ToList();
            }

            if (!string.IsNullOrEmpty(createdName))
            {
                getData = getData.Where(c => c.CreatedName.Contains(createdName)).ToList();
            }
            ...
            var fileName = string.Format("TM OVERTIME LOG TRACKING {0:ddMMyyyy}-{1:ddMMyyyy}_{2:yyyyMMdd}.xlsx", startDate, endDate, DateTime.Now);
            ...
            using (var ms = new MemoryStream())
            {
                using (var package = new ExcelPackage())
                {
                    ...
                    var cols = new[] { "Noreg", "Name", "PostName", "JobName", "Class", "Division", "Department", "Section", "Line", "Group", "Date", "Proxy In", "Proxy Out", "Type", "Code or Hour", "Activity", "Reason", "Created Date", "Created By" };
                    ...
                    foreach (var item in getData)
                    {
                        ...
                        sheet.Cells[rowIndex, 11].Value = item.Date.HasValue ? item.Date.Value.ToString("dd/MM/yyyy") : "-";
                        sheet.Cells[rowIndex, 12].Value = item.ProxyIn.HasValue ? item.ProxyIn.Value.ToString("dd/MM/yyyy HH:mm") : "-";
                        sheet.Cells[rowIndex, 13].Value = item.ProxyOut.HasValue ? item.ProxyOut.Value.ToString("dd/MM/yyyy HH:mm") : "-";
                        sheet.Cells[rowIndex, 14].Value = item.Type;
                        sheet.Cells[rowIndex, 15].Value = item.CodeOfHour;
                        sheet.Cells[rowIndex, 16].Value = item.Activity;
                        sheet.Cells[rowIndex, 17].Value = item.Reason;
                        sheet.Cells[rowIndex, 18].Value = item.CreatedDate.HasValue ? item.CreatedDate.Value.ToString("dd/MM/yyyy") : "-";
                        sheet.Cells[rowIndex, 19].Value = item.CreatedName;
    ...
    public class OvertimeLogTrackingController : MvcControllerBase
    {
