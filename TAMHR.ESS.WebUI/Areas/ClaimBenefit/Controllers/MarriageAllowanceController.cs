...
namespace TAMHR.ESS.WebUI.Areas.ClaimBenefit.Controllers
{
    ...
    /// <summary>
    /// Marriage allowance API controller.
    /// </summary>
    [Route("api/" + ApplicationForm.MarriageAllowance)]
    public class MarriageAllowanceApiController : FormApiControllerBase<MarriageAllowanceViewModel>
    {
        ...
        protected override void ValidateOnCreate(string formKey)
        {
            ...
            ClaimBenefitService.PreValidateMarriage(ServiceProxy.UserClaim.NoReg);
        }

        //private DataSourceResult GetDetailsReport(DataSourceRequest request, DateTime startDate, DateTime endDate)
        //{
        //    return ServiceProxy.GetQueryDataSourceResult<General>(@"SELECT i.*, u.Name, u.PostName FROM dbo.TB_M_BENEFIT_CLAIM_GENERAL i LEFT JOIN dbo.VW_USER_POSITION u ON u.NoReg = i.NoReg WHERE i.AllowanceType IN ('marriageallowance') AND i.TransactionDate BETWEEN @startDate AND @endDate", request, new { startDate, endDate });
        //}
        ...
        {
            var letterOfGuarantee = ClaimBenefitService.GetDocumentRequestDetails<MarriageAllowanceViewModel>("marriage-allowance", startDate, endDate);
        ...
        public IActionResult Download(DateTime startDate, DateTime endDate)
        {
            ...
            var output = ClaimBenefitService.GetDocumentRequestDetails<MarriageAllowanceViewModel>("marriage-allowance", startDate, endDate);

            var fileName = string.Format("MARRIAGE ALLOWANCE REPORT {0:ddMMyyyy}-{1:ddMMyyyy}.xlsx", startDate, endDate);
            ...
            using (var ms = new MemoryStream())
            {
                using (var package = new ExcelPackage())
                {
                    ...
                    foreach (var data in output)
                    {
                        ...
                        sheet.Cells[rowIndex, 4].Value = item.AmountAllowance;
                        sheet.Cells[rowIndex, 5].Value = item.WeddingDate;
