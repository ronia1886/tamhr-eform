...
namespace TAMHR.ESS.WebUI.Areas.ClaimBenefit.Controllers
{
    ...
    /// <summary>
    /// Company loan API controller.
    /// </summary>
    [Route("api/" + ApplicationForm.CompanyLoan)]
    [ApiController]
    public class LoanApiController : FormApiControllerBase<LoanViewModel>
    {
        ...
        protected override void ValidateOnCreate(string formKey)
        {
            ...
            claimBenefitService.PreValidateCompanyLoan7Up(ServiceProxy.UserClaim.NoReg, ServiceProxy.UserClaim.Name);
        }

        protected override void ValidateOnPostCreate(DocumentRequestDetailViewModel<LoanViewModel> requestDetailViewModel)
        {
            base.ValidateOnPostCreate(requestDetailViewModel);

            if ((int.Parse(requestDetailViewModel.Object.CalculationLoan) * requestDetailViewModel.Object.BasicSalary) < requestDetailViewModel.Object.LoanAmount)
                throw new Exception("Cannot input loan more than max ammount " + (int.Parse(requestDetailViewModel.Object.CalculationLoan) * requestDetailViewModel.Object.BasicSalary).ToString());
        }

        protected override void ValidateOnPostUpdate(DocumentRequestDetailViewModel<LoanViewModel> requestDetailViewModel)
        {
            base.ValidateOnPostCreate(requestDetailViewModel);
        ...
        [HttpPost("verified")]
        public async Task<IActionResult> Verified(string eventName, [FromBody]DocumentRequestDetailViewModel<LoanViewModel> documentRequestDetail)
        {
            //update
            ...
            claimBenefitService.InsertAllowanceSeq(docApp.CreatedBy, documentRequestDetail.Object);
        ...
        public System.IO.MemoryStream GeneratePaymentSchedule(Guid id)
        {
            try
            {
                var app = approvalService.GetDocumentRequestDetailViewModel<LoanViewModel>(id, "");
        ...
        {
            return ServiceProxy.GetQueryDataSourceResult<CompanyLoan>(@"SELECT i.*, u.Name, u.PostName FROM dbo.TB_M_BENEFIT_CLAIM_COMPANY_LOAN i LEFT JOIN dbo.VW_USER_POSITION u ON u.NoReg = i.NoReg WHERE i.CreatedOn BETWEEN @startDate AND @endDate", request, new { startDate, endDate });
        }
        ...
        public IActionResult Download(DateTime startDate, DateTime endDate)
        {
            ...
            //var output = GetDetailsReport(request, startDate, endDate).Data.Cast<CompanyLoan>();
            var output = claimBenefitService.GetDocumentRequestDetails<LoanViewModel>("company-loan", startDate, endDate);

            var fileName = string.Format("COMPANY LOAN REPORT {0:ddMMyyyy}-{1:ddMMyyyy}.xlsx", startDate, endDate);
            ...
            using (var ms = new MemoryStream())
            {
                using (var package = new ExcelPackage())
                {
                    ...
                    foreach (var data in output)
                    {
                        ...
                        sheet.Cells[rowIndex, 4].Value = item.LoanType;                         
                        sheet.Cells[rowIndex, 5].Value = item.CostNeeds?.ToString("N0");        
                        sheet.Cells[rowIndex, 6].Value = item.LoanAmount?.ToString("N0");       
                        sheet.Cells[rowIndex, 7].Value = item.LoanPeriod;                       
                        sheet.Cells[rowIndex, 8].Value = item.BasicSalary?.ToString("N0");      
                        sheet.Cells[rowIndex, 9].Value = item.Interest.ToString("N2");          
                        sheet.Cells[rowIndex, 10].Value = item.Period?.ToString("dd/MM/yyyy");  
